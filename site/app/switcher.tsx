...
export const Switcher: React.FC<SwitcherProps> = ({
  ...
  return (
    <div className={clsx(
      'relative flex backdrop-blur-md rounded-full p-1.5 min-w-[140px] h-[40px] transition-all duration-300',
      'border-2 shadow-md',
      isDarkTheme 
        ? 'bg-gray-900/80 border border-gray-700/50' 
        : 'bg-white/100 border border-white/100'
    )}>
      {options.map((option) => (
        <button
          key={option.id}
          ...
          className={clsx(
            ...
            isDarkTheme 
              ? (activeOption === option.id ? 'text-white' : 'text-gray-300 hover:text-gray-100')
              : (activeOption === option.id ? 'text-gray-900' : 'text-gray-500 hover:text-gray-700')
          )}
        >
          {option.icon && (
